  ...
  fetch("http://localhost:4000/library/", {
    ...
    .then((data) => {
      ...
      data.forEach((book) => {
        innerString += `
          <tr>
          <td><a href="book.html?id=${book.id}" class="book-id">${book.id}</a></td>
            <td>${book.title}</td>
            <td>${book.author}</td>
            <td>${book.genre}</td>
    ...
    Swal.fire({
      ...
      html: `
        <form id="bookForm">
          ...
          <input type="text" id="author" class="swal2-input">
          <input type="text" id="genre"  class="swal2-input">
          <input type="text" id="yearPublished" class="swal2-input">
        </form>
      `,
      showCancelButton: true,
